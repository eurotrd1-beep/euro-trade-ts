...
export function identify(request: Request, env: Env): Caller {
  const service = request.headers.get('x-service-secret');
  if (service && env.SERVICE_SECRET && secretEquals(service, env.SERVICE_SECRET)) {
    return { kind: 'service' };
  }
  const admin = request.headers.get('x-admin-secret');
  if (admin && env.ADMIN_SECRET && secretEquals(admin, env.ADMIN_SECRET)) {
    return { kind: 'admin' };
  }
